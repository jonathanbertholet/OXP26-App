...
from scrape_oxp import EVENTS, BASE, Fetcher, fetch_tags, parse_listing, parse_track_detail, slim_track
...
def merge_listing(previous, listing, tags):
    old = {t['id']: t for t in previous['tracks']}
    live = {t['id'] for t in listing}
    active = {t['id'] for t in previous['tracks'] if not t.get('unavailable')}
    assert listing and len(live) == len(listing), 'Empty or duplicate listing'
    assert len(active - live) <= max(2, len(active) * .10), 'More than 10% of talks disappeared; review source'
    dated = sum(bool(t.get('starts_at')) for t in listing)
    old_dated = sum(bool(t.get('starts_at')) for t in previous['tracks'] if not t.get('unavailable'))
    ...
    for row in listing:
        ...
        merged.append(track)
    merged.extend({**copy.deepcopy(old[i]), 'unavailable': True} for i in sorted(set(old) - live))
    return merged
...
def update_event(previous, spec, fetcher, tags, now):
    ...
    tracks = merge_listing(previous, listing, tags)
    def detail(track):
    ...
    result['tags'] = tags
    result['locations'] = sorted({t['location'] for t in listing if t.get('location')})
    result['event']['source_checked_at'] = now.isoformat(timespec='seconds')
    days = sorted({t['day'] for t in listing if t.get('day')})
    if days:
